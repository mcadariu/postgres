...
#define pgstat_count_buffer(rel, metadata, hit)					                        \
	do {																				\
		if (pgstat_should_count_relation(rel)) {                    					\
			(rel)->pgstat_info->counts.metadata_blocks_fetched += (metadata);           \
			(rel)->pgstat_info->counts.record_blocks_fetched += !(metadata);            \
            (rel)->pgstat_info->counts.metadata_blocks_hit += (metadata) & (hit);       \
            (rel)->pgstat_info->counts.record_blocks_hit += (!(metadata)) & (hit);      \
	   }																				\
	} while (0)
